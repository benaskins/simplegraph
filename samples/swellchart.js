(function($) {
  ...
  $(function() {
    ...
    $(".swellgraph .chart").each( function() {
      ...
      var settings = swellChartDefaults();
      ...
      var swellchart = new SimpleGraph(wave_height_am, forecast_day, canvas, settings);

      // Extend the graph with custom methods
      $.extend(swellchart, {
        ...
        labelYAxis: function() {
          ...
          for (var i = start_from, ii = (this.grid.maxValueYAxis); i <= ii; i = i + increment) {
            ...
            canvas.text(x, y, value + "ft").attr(this.settings.yAxisLabelStyle);        
          }
        },
                
        plotSwellDirections: function(swell_direction) {
          // Add the swell direction rectangle
          this.canvas.rect(swellchart.grid.leftEdge, 225, 285 - this.grid.leftEdge, 40).attr({stroke: "none", fill: "#fff"});
        ...
        plotWindDirections: function(wind_direction_am, wind_direction_pm) {
          ...
          canvas.rect(this.grid.leftEdge, 275, 285 - this.grid.leftEdge, 40).attr({stroke: "none", fill: "#fff"});
          canvas.image("images/chart-wind.gif", this.grid.leftEdge - 15, 285, 8, 19);
          // Plot the directions
      ...
      // Draw the chart
      swellchart.drawBackground();
      swellchart.drawYAxisSightLines();
      swellchart.plotAmWaveHeights();
      swellchart.plotPmWaveHeights();
      swellchart.labelYAxis();
      swellchart.plotSwellDirections(swell_direction);
      swellchart.plotWindDirections(wind_direction_am, wind_direction_pm);
    });
  ...
  function swellChartDefaults() {
    var settings = $.extend({}, $.fn.simplegraph.defaults, {
      width: 300,
      height: 215,
      drawLine: false,
      drawBars: true,
